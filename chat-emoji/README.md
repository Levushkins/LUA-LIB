...
| `0x100CB640` | 16 | буквы-плашки |

Итого **1329** смайлов в порядке, совпадающем со стандартным Unicode/CLDR.
...
они идут отдельной группой «Сервер» в конец, поэтому итоговых записей 1386.
...
Панель — это поиск по имени плюс сетка, разбитая на восемь вкладок в том же
порядке, что и в чате: «Смайлы и животные», «Люди и жесты», «Праздники и
предметы», «Еда и растения», «Транспорт и места», «Символы», «Буквы» и
«Сервер». Поиск понимает и синонимы — `:)`, `<3` и прочие текстовые шорткаты.
