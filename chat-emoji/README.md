...
Все файлы здесь сохранены в **UTF-8**, а ImGui как раз ждёт UTF-8 — поэтому
русские строки уходят в него **как есть, без `u8()`**. Привычная обёртка
`u8'текст'` нужна ровно наоборот: для скриптов, сохранённых в cp1251.
