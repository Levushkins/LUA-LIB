...
- Пересчёт стилей идёт только когда что-то поменялось: наведение мыши, клик,
  изменение DOM или state (плюс каждый кадр, пока играет `transition`).
- Правила разложены по корзинам (id / класс / тег), поэтому проверяются не все
  селекторы подряд.
- Ширины текста кешируются по (шрифт, размер, строка) в пределах кадра.
- Раскладка пересчитывается только при изменении дерева, размеров или прокрутки.

Практический ориентир: меню на ~200 элементов пересчитывается за доли
миллисекунды на кадрах без изменений и не грузит игру.
